...
public interface CRYPTO_set_mem_functions$m {
    ...
    static CRYPTO_set_mem_functions$m ofAddress(MemoryAddress addr, ResourceScope scope) {
        NativeSymbol symbol = NativeSymbol.ofAddress("CRYPTO_set_mem_functions$m::" + Long.toHexString(addr.toRawLongValue()), addr, scope);return (long x0, jdk.incubator.foreign.MemoryAddress x1, int x2) -> {
            try {
                return (jdk.incubator.foreign.MemoryAddress)constants$70.CRYPTO_set_mem_functions$m$MH.invokeExact(symbol, x0, x1, x2);
            } catch (Throwable ex$) {
